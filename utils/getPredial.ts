...
export const getPredialXml2Json = (xmlSoap: string) => {
  const options = {
  ...
  try {
    ...
    const item =
      body["CreaPasePredialPlayaWS.ExecuteResponse"] || body.ExecuteResponse;
    ...
    // Extraer valores
    const codeReference = item["Referencia"] ?? 0;
    const errorDescription = item["Errordescripcion"] ?? null;
